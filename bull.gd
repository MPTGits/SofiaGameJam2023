...
func _physics_process(delta):
	speed = 1.5 * original_speed  if get_tree().get_first_node_in_group("speed") else original_speed
	var targets = get_tree().get_nodes_in_group("server")
	for t in targets:
		if t.is_broken == false:
			target = t
			break;
	if target == null:
